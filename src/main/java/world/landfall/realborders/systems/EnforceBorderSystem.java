...
public class EnforceBorderSystem extends DelayedEntitySystem<EntityStore> {
    ...
    public void tick(float dt, int index, @NonNull ArchetypeChunk<EntityStore> archetypeChunk, @NonNull Store<EntityStore> store, @NonNull CommandBuffer<EntityStore> commandBuffer) {
        ...
        var hurtLevel = hurtLevel(c, r, pos);
        switch(hurtLevel) {
            case 0 -> {}
            case 1 -> {}
        ...
        if (hurtLevel > 0) {
            world.execute(() -> {
                ...
                SoundUtil.playSoundEvent2d(ref, SoundEvent.getAssetMap().getIndex("SFX_Portal_Neutral_Open"), SoundCategory.SFX, BORDER_VOLUME / (6-hurtLevel), 1, store);
